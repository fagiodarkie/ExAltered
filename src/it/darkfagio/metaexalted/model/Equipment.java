...
public class Equipment {
	...
	public String toString() {
		return Sequences.toString(item, ",-,") + "/" + Sequences.toString(armor, ",-,") + "/"
				+ Sequences.toString(weapon, ",-,") + "/" + Sequences.toString(equippedItems, ",-,")
				+ "/" + Sequences.toString(equippedArmor, ",-,") + "/" + Sequences.toString(equippedWeapon, ",-,") + "/" + (weaponInUse + 1);
	}
	
	public void fromString(String string) {
		String[] res = string.trim().split("/");
		if (res.length < 6) return;
		String[] its = Sequences.divideStringPer(res[0], ",-,");
		String[] arms = Sequences.divideStringPer(res[1], ",-,");
		String[] weps = Sequences.divideStringPer(res[2], ",-,");
		String[] eqItems = Sequences.divideStringPer(res[3], ",-,");
		equippedArmor = Sequences.parseIntFromString(res[4], ",-,");
		String[] eqWep = Sequences.divideStringPer(res[5], ",-,");
		weaponInUse = Short.parseShort(res[6]) - 1;
		for (int i = 0; i < its.length; ++i) {
			item.add(Item.factoryFromString(its[i]));
		}
		for (int i = 0; i < arms.length; ++i) {
			armor.add(Armor.factoryFromString(arms[i]));
		}
		for (int i = 0; i < weps.length; ++i) {
			weapon.add(Weapon.factoryFromString(weps[i]));
		}
		for (int i = 0; i < eqItems.length; ++i) {
			equippedItems.add(Integer.parseInt(eqItems[i]));
		}
		for (int i = 0; i < eqWep.length; ++i) {
			equippedWeapon.add(Integer.parseInt(eqWep[i]));
		}
	}
