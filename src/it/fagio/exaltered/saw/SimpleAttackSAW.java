...
import it.fagio.exaltered.model.Movement;
import it.fagio.exaltered.model.PlayCharacter;
import it.fagio.exaltered.model.Virtues;
import it.fagio.exaltered.model.Weapon;
import it.fagio.exaltered.panels.CombatStatsPanel;
...
public class SimpleAttackSAW extends SAWPanel {
	...
	public SimpleAttackSAW(SolveAttackWizard solveAttackWizard, boolean b,
		...
		specBox 		= new JCheckBox("Specialit� applicabile");
	...
	private void remodel() {
		...
		labels.add(new JLabel("Abilit� utilizzata:"));
		...
		comboBoxes.add(new JLabel());
		if (!attacking) {
			labels.add(new JLabel("Virt� dell'azione perfetta: "));
			comboBoxes.add(virtueBox);
		}
		labels.add(new JLabel("Moti spesi: "));
		comboBoxes.add(spentMotes);
		labels.add(new JLabel("Volont� spesa: "));
