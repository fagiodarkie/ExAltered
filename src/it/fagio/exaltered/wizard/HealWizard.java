...
import it.fagio.exaltered.panels.HealthLimbGui;
import it.darkfagio.utils.Format;
import it.darkfagio.utils.gui.DialogPanel;

public class HealWizard extends DialogPanel {
	...
	public HealWizard(PlayCharacter c, int pos, HealthLimbGui healthLimbGui) {
		gui = healthLimbGui;
		this.ch = c;
		this.position = pos;
		...
		JLabel label2 = new JLabel("Quanti livelli vuoi curare?");
		...
		ok.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (healed > 0) {
					ch.getHealth().get(position).heal(healed, (short)levelToHeal);
					gui.notifyChange();
				}
				superDialog.dispose();
			}
	...
	public String getTitle() {
		return "Cura " + HealthManager.NAME[gui.getPosition()];
	}
